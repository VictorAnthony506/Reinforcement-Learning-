...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    """
    p = np.random.random()
    Q = q_func[state_1, state_2, :]
    action_index, object_index = None, None
    if p < epsilon:
        action_index = np.random.randint(0, NUM_ACTIONS)
        object_index = np.random.randint(0, NUM_OBJECTS)
    else:
        action_index, object_index = np.unravel_index(np.argmax(Q, axis=None), Q.shape)
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    """
    if not terminal:
        Q = q_func[current_state_1, current_state_2, action_index, object_index]
        q_max = np.max(q_func[next_state_1, next_state_2, :])
        q_func[current_state_1, current_state_2, action_index, object_index] = \
            ((1-ALPHA)*Q) + ALPHA*(reward+(GAMMA*q_max))
    elif terminal:
        # collect final reward and don't look ahead.
        Q = q_func[current_state_1, current_state_2, action_index, object_index]
        q_func[current_state_1, current_state_2, action_index, object_index] = \
            ((1 - ALPHA) * Q) + (ALPHA * reward)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        next_state1 = dict_room_desc[nrd]
        next_state2 = dict_quest_desc[nqd]

        if for_training:
            # update Q-function.
            tabular_q_learning(q_func, state_1, state_2, action_index,
                               object_index, reward, next_state1, next_state2, terminal)
