...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if p < epsilon:
        ...
        action_index, object_index = index2tuple(np.argmax(Q, axis=None))
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    current_q_value = (theta @ current_state_vector)[tuple2index(action_index, object_index)]
    next_q_max = np.max(next_q_value)
    if not terminal:
        g = (reward + (GAMMA * next_q_max) - current_q_value) * current_state_vector
        theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index,
                                                                           object_index)] + ALPHA * g
    elif terminal:
        g = (reward - current_q_value)*current_state_vector
        theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)] + ALPHA * g
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        nrd, nqd, reward, terminal = framework.step_game(current_room_desc, current_quest_desc, action_index, object_index)
        next_state = nrd + nqd
        ...
        # prepare next step
        current_room_desc = nrd
        current_quest_desc = nqd
