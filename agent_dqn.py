...
def epsilon_greedy(state_vector, epsilon):
    ...
    if p < epsilon:
        ...
        with torch.no_grad():
            ...
            action_index = np.argmax(q_values_action_next, axis=None)
            object_index = np.argmax(q_values_object_next, axis=None)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    Returns:
        None
    """
    # q_values_action_next.max() is the maximum q_value over all actions
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        next_state_vector = torch.FloatTensor(
            utils.extract_bow_feature_vector(nrd+nqd, dictionary))
        # TODO Your code here
